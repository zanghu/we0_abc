...
const CodeBlock = memo(
  ({
    ...
    // 缓存语法高亮样式
    const lightStyle: SyntaxHighlighterProps['style'] = useMemo(() => ({
      ...vscDarkPlus,
      'pre[class*="language-"]': {
      ...
      string: { color: '#0a3069', fontWeight: '500' },
      keyword: { color: '#cf222e', fontWeight: '600' },
      function: { color: '#6f42c1', fontWeight: '600' },
      number: { color: '#0a3069', fontWeight: '500' },
      operator: { color: '#24292f' },
      variable: { color: '#953800', fontWeight: '500' },
      className: { color: '#116329', fontWeight: '600' },
      punctuation: { color: '#24292f' },
      property: { color: '#953800', fontWeight: '500' },
    }), []);

    const darkStyle: SyntaxHighlighterProps['style'] = useMemo(() => ({
      ...vscDarkPlus,
      'pre[class*="language-"]': {
        background: 'transparent',
        margin: 0,
    ...
    const syntaxStyle = useMemo(() => {
      if (typeof window === 'undefined') return lightStyle;
      return window.matchMedia('(prefers-color-scheme: dark)').matches ? darkStyle : lightStyle;
    ...
    return (
      <div className="my-4">
        <div className="rounded-lg bg-gray-50 dark:bg-[#1e1e1e] overflow-hidden group border border-gray-200 dark:border-[#333] shadow-sm">
          <div className="flex items-center justify-between px-4 py-2 border-b border-gray-200 dark:border-[#333] bg-gray-100/50 dark:bg-[#2d2d2d]">
            <div className="flex items-center gap-2.5">
              {filePath ? (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-gray-600 dark:text-gray-400 font-medium">{filePath}</span>
                </div>
              ) : language ? (
                <div className="text-sm text-gray-600 dark:text-gray-400 font-medium">{language}</div>
              ) : null}
            </div>
            <button
              onClick={handleCopy}
...
export const MessageItem: React.FC<MessageItemProps> = ({
  ...
  const avatarColor = isUser ? "bg-purple-500 dark:bg-purple-600" : "bg-gray-100 dark:bg-[rgba(45,45,45)]";
            ...
            <div className="text-gray-900 dark:text-gray-100 leading-relaxed prose dark:prose-invert prose-sm max-w-none">
              <ReactMarkdown
                components={{
                  ...
                  li({ children }) {
                    return <li className="text-gray-700 dark:text-gray-300">{processChildren(children)}</li>;
                  },
                  a({ children, href }) {
                    return (
                      <a
